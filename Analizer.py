...
def analize_file(data_file=None):
    ...
    if not os.path.exists(results_path):
        ...
        print("Done.")
        print("Cleaning data...")
        covid_df = merge_clean_data(covid_df= covid_df, mappings=mappings, catalogs=catalogs)
        print("Done.")
        # print(covid_df.head(5))  # Prints the top n values of my data source
        # Save clean data
        covid_df.to_excel(results_path, index=False, header=True)
    else:
        print("File {} exist, loading file...".format(results_path))
        xl = pd.ExcelFile(results_path)
        print("Parsing 'Sheet1'...")
...
def merge_clean_data(covid_df, mappings, catalogs):
...
# sex_by_state_df = covid_df[['edad', 'sexo', "entidad_res"]]
# print(covid_df)
